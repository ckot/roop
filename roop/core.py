...
def parse_args2() -> None:
    signal.signal(signal.SIGINT, lambda signal_number, frame: destroy())
    program = argparse.ArgumentParser(formatter_class=lambda prog: argparse.HelpFormatter(prog, max_help_position=100))
    program.add_argument('-g', '--group-by-src', help="each src image gets it's output in output_{src} dir", action="store_true", dest='group_by_src')
    program.add_argument('-c', '--clobber', help="clobber existing output files", action="store_true", dest='clobber')
    program.add_argument('-o', '--output-dir', help="output_dir", dest="output_dir")
...
def start() -> None:
    ...
    # validate video
    # I think there might be a race condition, perhaps due to my using google drive
    time.sleep(1.0)
    # if "video" == roop.globals.output_format:
